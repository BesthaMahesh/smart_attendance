...
export default function StudentRegisterPage() {
  ...
  const [samples, setSamples] = useState([]); // Array of base64 screenshots
  ...
  const handleStartCapture = (e) => {
    ...
    if (!name || !rollNumber || !email) {
      setErrorMessage("Please fill out all student profile fields first.");
      return;
    }
    setErrorMessage('');
    setSamples([]);
  ...
  const handleRegister = async () => {
    ...
    try {
      const res = await api.post('/auth/register-student-face', {
        name,
        rollNumber,
        email,
        department,
      ...
      setSuccessMessage("Roster profile created successfully!");
      ...
      console.error(err);
      setErrorMessage(err.response?.data?.error || 'Failed to register student face profile.');
      setLoading(false);
    }
  };
          ...
          <div className="p-8 border-r border-slateCustom-200 flex flex-col justify-between">
            <div className="space-y-6">
              <div className="space-y-1">
                <h1 className="text-2xl font-bold text-slateCustom-900 font-display">Student Enrollment</h1>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col space-y-1">
                  <label className="text-xs font-semibold text-slateCustom-500">Roll Number</label>
                  <input
                    type="text"
                    required
                    value={rollNumber}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col space-y-1">
                    <label className="text-xs font-semibold text-slateCustom-500">Academic Year</label>
                    <select
                      value={year}
